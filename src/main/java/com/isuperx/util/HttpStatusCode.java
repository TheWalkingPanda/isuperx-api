...
public class HttpStatusCode {
	public static final int OK 						= 200;	//[GET]：			服务器成功返回用户请求的数据，该操作幂等
	...
	public static final int INVALID_REQUEST 		= 400;	//[POST/PUT/PATCH]：用户发出的请求有错误，服务器没有进行新建或修改数据的操作，该操作幂等
	public static final int UNAUTHORIZED 			= 401;	//[*]：				表示用户没有权限（令牌、用户名、密码错误）
	public static final int FORBIDDEN 				= 403;	//[*]：				表示用户得到授权（与401错误相对），但是访问被禁止
	public static final int NOT_FOUND 				= 404;	//[*]：				用户发出的请求针对的是不存在的记录，服务器没有进行操作，该操作幂等
